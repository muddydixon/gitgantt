...
export default class ModifyIssueForm extends Component {
  onUpdate(project, issue){
    const begin = this.refs.begin.value.trim(),
          end = this.refs.end.value.trim(),
          milestoneId = this.refs.milestone.value.trim();
    IssueAction.update(issue.id, {begin, end, milestoneId});
  }
  render(){
    ...
    return <div className="static-modal">
      <Modal.Dialog>
            ...
            <div className="form-group row">
              ...
              <div className="col-xs-10">
                <select ref="milestone" className="form-control" defaultValue={(issue.milestone && issue.milestone.title) || "unscheduled"} >
                  ...
                  {milestones.map((m, id)=> <option key={id} value={m.id}>{m.title}</option>)}
                </select>
              </div>
            </div>
          </form>
        ...
        <Modal.Footer>
          <button className="btn btn-info" onClick={(e)=> this.onUpdate(project, issue)}>Update</button>
