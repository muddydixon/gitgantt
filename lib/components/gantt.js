...
import ModifyIssueForm from "./modify-issue";
...
const setDates = (issue)=>{
  const head = (typeof issue.body !== "undefined" ? issue.body : issue.description)
          .split(/\n/).slice(0,3).join("\n");

  const b = head.match(/^\*\sbegin:\s*(.+)/m);
  const e = head.match(/^\*\send:\s*(.+)/m);
  const duration = getDurationFromLabel(issue);
  issue.begin = b ? moment(b[1].replace(/\//g, "-")) : moment(issue.created_at).utcOffset(+540);
...
export default class Gantt extends Component {
  ...
  componentWillMount(){
    const projectId = this.props.params.user ?
            `${this.props.params.user}/${this.props.params.repo}` :
            this.props.params.projectId;
    MileStoneAction.fetchAll(projectId);
    IssueAction.fetchAll(projectId);
  }
  onModifyIssue(issue){
    this.setState({
      modifingIssue: issue
  ...
  onUpdate(issue, refs){
    const projectId = this.props.params.user ? `${this.props.params.user}/${this.props.params.repo}` :
            this.props.params.projectId;
    const begin = refs.begin.value.trim(),
          end = refs.end.value.trim(),
          milestoneId = refs.milestone.value.trim();
    IssueAction.update(projectId, issue.id, {begin, end, milestoneId});
  }
  render(){
    ...
    const project = projects.filter((p)=> `${p.id}` === projectId || p.full_name === projectId)[0];
    ...
    const organizedIssues = d3.nest()
            ...
            .key((i)=> (i.assignee && (i.assignee.login || i.assignee.name)) || "unassigned")
            .entries(issues);

    return <div className="container">
      <ModifyIssueForm issue={this.state.modifingIssue}
        project={project}
        milestones={milestones}
        onUpdate={this.onUpdate.bind(this)}
        onCloseModifyIssue={this.onCloseModifyIssue.bind(this)}
      />
      <div className="row">
        <div className="row">
      ...
      {organizedIssues.map((milestone, id)=> <MileStone
                           ...
                           onModifyIssue={this.onModifyIssue.bind(this)}
